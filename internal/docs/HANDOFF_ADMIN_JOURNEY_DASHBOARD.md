...
> **Status (2026-08-18, dev session): implemented in 1.23.0.**
> ops_metric_journey (metric-grain, one row ALWAYS; stage columns left-to-right; '; '-joined report ties; published_collibra / published_pbi_writeback from gov_publish_log successes) and ops_report_journey (report-grain; workspace NAMES; proc lists; tie_kind incl. corpus membership) — both registry contracts materialized by 500_validate from contract tables only. Decision (b): the workspace collector now fetches display names (one call/workspace/run, tolerant fallback to id) and input_report_sources carries workspace_name. Decision (c): described_status uses the funnel's rejected_by_agent code; parse error_category passes through. Reconciliation tests pin loaded = parsed + errored and both grain rules. Your build steps: internal/docs/RUNBOOK_JOURNEY_DASHBOARD.md (the four visuals, numbered). Note: 500 must run after a 060+publisher cycle for ties/publish flags to populate; empty is honest before that.
