...
def test_blocks_covered_path_without_brief(project):
    r = edit(project, "aivia/flows/bar.py")
    assert r.returncode == 2
    assert "HARD GATE" in r.stderr
    assert "aivia/flows/bar.py" in r.stderr


def test_allows_covered_path_declared_in_approved_brief(project):
    assert edit(project, "aivia/flows/foo.py").returncode == 0
...
def test_closed_and_presented_briefs_do_not_unlock(project):
    assert edit(project, "aivia/flows/closed.py").returncode == 2
    assert edit(project, "aivia/flows/draft.py").returncode == 2
