...
def test_uses_table_closure_matches_certified_answer_key():
    ...
    assert len(tables_of("reports.USP_Severe_Sepsis")) == 35
    assert len(tables_of("reporting.USP_ED_Sepsis")) == 47
    assert len(tables_of("reports.USP_ED_Sepsis")) == 47
    ...
    assert len({e["targetId"] for e in c2t if e["sourceId"] == severe}) == 121
