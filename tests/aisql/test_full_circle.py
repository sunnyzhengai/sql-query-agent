...
containment law (no aisql module imports the old src tree).
...
def test_landed_file_matches_the_binding(circle):
    ...
    assert rows[0]["Description"].startswith("AISQL agent generated: ")
...
def test_aivia_is_self_contained_no_src_imports():
    ...
    offenders = []
    for path in sorted((ROOT / "aisql").rglob("*.py")):
