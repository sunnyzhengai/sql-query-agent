...
def test_two_alias_corpse_voices_both_filters(shaken):
    ...
    from aisql.flows import produce as _produce
    store, _, _ = shaken
...
def test_bpa_corpse_on_clause_filter_is_membership(shaken):
    ...
    from aisql.flows import produce as _produce
    from aisql.lenses import census as _census
    store, _, _ = shaken
    floor = _produce.compose_floor(ReadApi(store),
    ...
    assert gc["outer_join_conditions_not_voiced"] == 130  # counted, declared
