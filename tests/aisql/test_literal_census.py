...
literal collection of 3+ strings in aisql/ carries a marker naming
...
AISQL = pathlib.Path(__file__).resolve().parents[2] / "aisql"
...
def scan():
    unmarked, badclass = [], []
    for py in sorted(AISQL.rglob("*.py")):
        src = py.read_text()
        lines = src.splitlines()
        tree = ast.parse(src)
        for node in _collections(tree):
            tag = _marked(lines, node.lineno)
            where = f"{py.relative_to(AISQL.parent)}:{node.lineno}"
            if tag is None:
