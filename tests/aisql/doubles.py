...
the live model (AISQL_RECORD=1) into a committed fixture and
...
def recorded_embed(texts):
    ...
    if missing:
        if not os.environ.get("AISQL_RECORD"):
            raise RecordingGap(
                f"{len(missing)} text(s) hold no recorded vector — "
                f"first: {missing[0]!r}. Run AISQL_RECORD=1 pytest "
                f"to record against the real {EMBED_MODEL}, then "
                f"commit {FIXTURE.name}")
        from aisql.console import _env_key, make_embedder
