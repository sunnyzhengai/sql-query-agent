...
(
	...
	INNER JOIN [EMRDB].[dbo].[HOSPITAL_ENCOUNTERS] hsp ON ENCS.ENCOUNTER_ID = hsp.ENCOUNTER_ID /*Developer C minimize to ENC_ID's we are looking at*/

	INNER JOIN [EMRDB].[dbo].[ADT_EVENTS] adtIn ON adtIn.ENCOUNTER_ID = hsp.ENCOUNTER_ID
...
SELECT DISTINCT
	...
	, ROW_NUMBER() OVER (PARTITION BY peh.ENCOUNTER_ID, vaplh.IN_DTTM ORDER BY vaplh.IN_DTTM, vaplh.OUT_DTTM) [inDeptRN]

	, ROW_NUMBER() OVER (PARTITION BY peh.ENCOUNTER_ID ORDER BY vaplh.IN_DTTM, vaplh.OUT_DTTM ) [ENC_ID Order]

INTO #Base_PopTemp

FROM #MainAdmDetails main

INNER JOIN [EMRDB].[dbo].[HOSPITAL_ENCOUNTERS] peh ON peh.ENCOUNTER_ID = main.ENCOUNTER_ID

INNER JOIN  vaplh ON vaplh.ENCOUNTER_ID = peh.ENCOUNTER_ID AND vaplh.ADT_DEPARTMENT_ID IS NOT NULL /*[EMRDB].[dbo].V_PATIENT_LOCATION_HISTORY*/
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY om.ENCOUNTER_ID ORDER BY mar.TAKEN_TIME) TIME_LINE
...
SELECT DISTINCT
	...
	, ROW_NUMBER() OVER(PARTITION BY base.ENCOUNTER_ID ORDER BY om.ORDER_DTTM ASC) AS TIME_LINE
...
SELECT base.ENCOUNTER_ID
	...
	, ROW_NUMBER() OVER(PARTITION BY base.ENCOUNTER_ID ORDER BY op.ORDER_TIME ASC) AS TIME_LINE
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY base.ENCOUNTER_ID ORDER BY op.ORDER_TIME, base.InDepartmentTime, ordR.RESULT_TIME ASC) AS TIME_LINE -- Developer C added Result time
...
SELECT base.ENCOUNTER_ID
	...
	, ROW_NUMBER() OVER(PARTITION BY base.ENCOUNTER_ID ORDER BY op.ORDER_TIME, res.RESULT_TIME ASC) AS TIME_LINE
...
SELECT
	...
	FROM #Base_Pop B -- ONLY THOSE PATIENTS WITH A POSITIVE SCORE
		...
		INNER JOIN EMRDB.dbo.MEDICATIONS CM ON CM.MEDICATION_ID = OM.MEDICATION_ID --AND CM.THERA_CLASS_CODE = 11 --Antibiotics
	...
	WHERE 1=1
		...
		AND OM.MED_ROUTE_CODE=11--IV ONLY

		AND MAI.MAR_ACTION_CODE IN ('1'			--GIVEN
...
DROP TABLE IF EXISTS #SSOrderSetOSQ_PRL;
	...
	FROM #Base_Pop B
		...
		INNER JOIN EMRDB.dbo.ORDER_TRACKING_METRICS OM ON OM.ENCOUNTER_ID = B.ENCOUNTER_ID
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY MAI.TAKEN_TIME ASC) TIME_LINE
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY C.ENCOUNTER_ID ORDER BY IFM.RECORDED_TIME ASC) AS TIME_LINE
...
SELECT DISTINCT
	...
	, ROW_NUMBER() OVER(PARTITION BY C.ENCOUNTER_ID ORDER BY ILN.PLACEMENT_INSTANT) TIME_LINE

INTO #ALLCVLTime

FROM #Base_Pop C

	INNER JOIN [EMRDB].[dbo].LINE_DEVICE_AIRWAY ILN ON ILN.ENCOUNTER_ID = C.ENCOUNTER_ID 
...
SELECT DISTINCT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY MAI.TAKEN_TIME) AS TIME_LINE

INTO #Pressors 

FROM #Base_Pop B

	LEFT JOIN EMRDB.dbo.MEDICATION_ORDERS OM ON OM.ENCOUNTER_ID = B.ENCOUNTER_ID
	...
	LEFT JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS PEH ON PEH.ENCOUNTER_ID = B.ENCOUNTER_ID
...
WHERE 1=1
	...
	AND MAI.ROUTE_CODE = 11 --INTRAVENOUS
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY OP.ORDER_TIME ASC) AS TIME_LINE
...
(
	...
	FROM #Base_Pop B
		...
		INNER JOIN [EMRDB].[dbo].PROCEDURE_ORDERS	 OP		 ON RESULTS.ORDER_PROC_ID = OP.ORDER_PROC_ID
														...
														AND OP.SPECIMEN_SOURCE_CODE=304				-- Lumber puncture
...
SELECT
	...
	ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY ILN.PLACEMENT_INSTANT) TIME_LINE

INTO #ETT

FROM #Base_Pop B

	INNER JOIN [EMRDB].[dbo].LINE_DEVICE_AIRWAY ILN ON ILN.ENCOUNTER_ID = B.ENCOUNTER_ID
...
FROM #BasePopABX A
	...
	LEFT OUTER JOIN 
		...
		(
			...
			[EMRDB].[dbo].V_PHARMACY_DISPENSE_ACTION VRDA WHERE VRDA.ACTION_TYPE_CODE=270
...
FROM #Base_Pop BP
	...
	INNER JOIN [EMRDB].[dbo].ED_ENCOUNTERS_DM DEE ON DEE.PATIENT_ID = BP.PATIENT_ID AND (DEE.ARRIVAL_DTTM BETWEEN BP.ED_DEPARTURE_TIME AND DATEADD(HH,24,BP.ED_DEPARTURE_TIME))
...
SELECT
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND BC.MBOrderTime IS NOT NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [IPSO SEVERE and Positive Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND BC.MBOrderTime IS NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END					AS [IPSO SEVERE and Positive Sepsis and Blood Culture NOT Ordered]
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND BC.MBOrderTime IS NOT NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END			AS [IPSO NON SEVERE and Positive Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND BC.MBOrderTime IS NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [IPSO NON SEVERE and Positive Sepsis and Blood Culture NOT Ordered]
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND BC.MBOrderTime IS NOT NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END			AS [IPSO SEVERE and Negative Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND BC.MBOrderTime IS NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [IPSO SEVERE and Negative Sepsis and Blood Culture NOT Ordered]
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND BC.MBOrderTime IS NOT NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [IPSO NON SEVERE and Negative Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND BC.MBOrderTime IS NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END					AS [IPSO NON SEVERE and Negative Sepsis and Blood Culture NOT Ordered]
...
MODIFICATION: PROCEDURE_ORDERS.PROC_CODE IS DEPRICATED AS OF APRIL 2019. REPLACING IT WITH PROCEDURE IDS.
	...
	--AND OP.PROC_CODE IN ('LAB006','LAB007') AND OP.SPECIMEN_SOURCE_CODE=304 
...
FROM  #Main sub
	...
	INNER JOIN EMRDB.dbo.ENCOUNTER_DIAGNOSES PED ON PED.ENCOUNTER_ID = SUB.ENCOUNTER_ID AND PED.LINE>1
...
cross apply (
			...
			FROM
				...
				INNER JOIN EMRDB..ADT_EVENTS ADTIN ON ADTIN.ENCOUNTER_ID = HSP.ENCOUNTER_ID
			...
			WHERE
				...
				HSP.ENCOUNTER_ID = #Main.ENCOUNTER_ID
...
FROM #Main BP 
	...
	INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS PEH ON PEH.ENCOUNTER_ID = BP.ENCOUNTER_ID --and bp.ENCOUNTER_ID=1016405505 
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY BP.ENCOUNTER_ID, BP.ADT_DEPARTMENT_ID, BP.IN_DTTM ORDER BY ifm.RECORDED_TIME ASC) AS TIME_LINE
...
FROM #Base_Pop BP 
	...
	LEFT OUTER JOIN
		...
		)ALERTACTIVATED ON ALERTACTIVATED.VISIT_ID = PEH.ENCOUNTER_ID
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.PROCEDURE_ORDERS OP  ON RESULTS.ORDER_PROC_ID = OP.ORDER_PROC_ID
				...
				--AND OP.PROC_CODE IN ('LAB006','LAB007') AND OP.SPECIMEN_SOURCE_CODE=304 --PER STEPHANIE, USE THOSE LAB CODES WITH SPECIMEN SOURCE AS LUMBAR PUNCTURE--('LAB005','LAB013','LAB004')
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.LINE_DEVICE_AIRWAY ILN ON ILN.ENCOUNTER_ID = B.ENCOUNTER_ID AND ILN.FLO_MEAS_ID='900112' AND ILN.PLACEMENT_INSTANT IS NOT NULL
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.LINE_DEVICE_AIRWAY ILN ON ILN.ENCOUNTER_ID = B.ENCOUNTER_ID AND ILN.FLO_MEAS_ID='900111' AND ILN.PLACEMENT_INSTANT IS NOT NULL
...
SELECT
	...
	CASE WHEN COUNT(IFM.RECORDED_TIME)>0 THEN 'Y' ELSE 'N' END AS PROPHYLAXIS_FLAG
...
SELECT
	...
	CASE WHEN COUNT(IFM.RECORDED_TIME)>0 THEN 'Y' ELSE 'N' END AS CVVH_FLAG
...
SELECT
	...
	CASE WHEN COUNT(IFM.RECORDED_TIME)>0 THEN 'Y' ELSE 'N' END AS OX_FLAG
...
SELECT
	...
	CASE WHEN COUNT(IFM.RECORDED_TIME)>0 THEN 'Y' ELSE 'N' END AS ECMO_FLAG
...
FROM #Main PD
	...
	(
		...
		WHERE
			...
			HNO.ENCOUNTER_ID = PD.ENCOUNTER_ID
...
INNER JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY VAPLH ON VAPLH.ENCOUNTER_NUM = PEH.ENCOUNTER_ID AND VAPLH.ADT_DEPARTMENT_ID IS NOT NULL
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY IFM.RECORDED_TIME ASC) AS TIME_LINE
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY OM.ENCOUNTER_ID ORDER BY MAI.TAKEN_TIME) TIME_LINE
...
SELECT
	...
	--, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID, B.ADT_DEPARTMENT_ID, B.IN_DTTM ORDER BY OM.ORDER_DTTM ASC) AS TIME_LINE

	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY OM.ORDER_DTTM ASC) AS TIME_LINE
...
SELECT DISTINCT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID, B.ADT_DEPARTMENT_ID, B.IN_DTTM ORDER BY ILN.PLACEMENT_INSTANT) TIME_LINE
...
FROM 
	...
	JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS PEH 
		...
		ON HTR.ENCOUNTER_ID = PEH.ENCOUNTER_ID

	JOIN EMRDB.dbo.PATIENTS PAT 

		ON PAT.PATIENT_ID = PEH.PATIENT_ID
...
WHERE
	...
	--and PEH.ENCOUNTER_ID=1018379064

	AND (PEH.INP_ADM_DATE IS NOT NULL --PATIENTS UST BE ADMITTED

		OR PEH.ED_DISPOSITION_CODE = 3 --ADMITTED

		or PEH.ADT_PATIENT_CLASS_CODE IN (101, 104)) 
...
SELECT
	...
	--, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY OM.ORDER_INST ASC) LINE
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY OP.ORDER_INST ASC) LINE
...
FROM
	...
	JOIN EMRDB.dbo.PROCEDURE_ORDERS OP
		...
		ON OP.ENCOUNTER_ID = B.ENCOUNTER_ID
...
INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS PEH ON HTR.ENCOUNTER_ID = PEH.ENCOUNTER_ID AND PEH.ADMIT_CONF_STAT_CODE IN (1,4) AND PEH.HOSP_ADMSN_TIME IS NOT NULL AND PEH.ADT_PATIENT_CLASS_CODE <> 102
...
AND MAI.MAR_ACTION_CODE IN (SELECT * FROM #MARActions)
...
SELECT b.ENCOUNTER_ID
	...
	, ROW_NUMBER() OVER(PARTITION BY b.ENCOUNTER_ID ORDER BY mai.TAKEN_TIME ASC) BOLUS_NUM
...
AND (mai.MAR_ACTION_CODE IN (SELECT * FROM #MARActions) AND mai.MAR_ACTION_CODE <> '99') --3/18/24 - Original code didn't have Rate Change action Stakeholder A believes there was a specific reason so leaving as it was 
...
SELECT B.ENCOUNTER_ID
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY MAI.TAKEN_TIME ASC) AS SS_LINE
...
INNER JOIN EMRDB.dbo.PROCEDURE_ORDERS op ON op.ENCOUNTER_ID = b.ENCOUNTER_ID
...
--INNER JOIN #Base_Pop PEH ON PEH.ENCOUNTER_ID = BP.ENCOUNTER_ID AND PEH.ADT_ARRIVAL_TIME IS NOT NULL--MAKE SURE ITS AN ED ARRIVAL--Arrival Time Filter addedd on 07.30.2019
...
/*TREATMENT PLAN*/
		...
		JOIN EMRDB.dbo.PROCEDURE_ORDERS OP ON OP.ENCOUNTER_ID = B.ENCOUNTER_ID
		...
		SELECT DISTINCT OM.ENCOUNTER_ID
		...
		JOIN EMRDB.dbo.MEDICATION_ORDERS OM ON OM.ENCOUNTER_ID = C.ENCOUNTER_ID
		...
		INNER JOIN EMRDB.dbo.MEDICATION_ORDERS OM ON OM.ENCOUNTER_ID = SC.ENCOUNTER_ID
		...
		INNER JOIN #ABX ABX ON ABX.ENCOUNTER_ID = OM.ENCOUNTER_ID

		INNER JOIN #BC BC ON BC.ENCOUNTER_ID = OM.ENCOUNTER_ID

		INNER JOIN #BOL2 BOL ON BOL.ENCOUNTER_ID = OM.ENCOUNTER_ID

		INNER JOIN EMRDB.dbo.MED_ADMIN_RECORDS MAI ON MAI.ORDER_MED_ID = OM.ORDER_MED_ID AND MAI.ROUTE_CODE = 11  --INTRAVENOUS
...
SELECT b.ENCOUNTER_ID
	...
	, op.ORDER_STATUS_CODE

	, ROW_NUMBER() OVER(PARTITION BY b.ENCOUNTER_ID ORDER BY om.ORDER_DTTM ASC) AS SS_LINE
...
JOIN EMRDB.dbo.ORDER_TRACKING_METRICS om ON om.ORDER_ID = op.ORDER_PROC_ID --AND OP.ORDER_STATUS_CODE<>4-- AND OP.INSTANTIATED_TIME IS NOT NULL AND OP.FUTURE_OR_STAND IS NULL-- ENCOUNTER_ID = B.ENCOUNTER_ID
...
/*CRITERIA 4*/
	...
	SELECT vpalh.ENCOUNTER_NUM AS ENCOUNTER_ID
	...
	INNER JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY vpalh ON vpalh.ENCOUNTER_NUM = b.ENCOUNTER_ID

	WHERE vpalh.EVENT_TYPE_CODE in (1, 3) --ADMISSION AND TRANSFER-IN
...
JOIN EMRDB.dbo.PROCEDURE_ORDERS op ON bp.ENCOUNTER_ID = op.ENCOUNTER_ID
...
SELECT c.ENCOUNTER_ID
	...
	, ROW_NUMBER() OVER(PARTITION BY c.ENCOUNTER_ID ORDER BY ABS(DATEDIFF(SS, ftz.FTZ, ifm.RECORDED_TIME))ASC) AS ORD_RESULTS_LINE
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY C.ENCOUNTER_ID ORDER BY ABS(DATEDIFF(MI, OP.ORDER_TIME, #FTZ.FTZ)) ASC) AS LINE
...
FROM 
	...
	JOIN #Base_Pop PEH ON PEH.ENCOUNTER_ID = C.ENCOUNTER_ID
...
FROM
	...
	INNER JOIN #Base_Pop PEH ON PEH.ENCOUNTER_ID = C.ENCOUNTER_ID
	...
	INNER JOIN EMRDB.dbo.HOSPITAL_ACCT_PX_LIST HAPL ON HAPL.HOSPITAL_ACCOUNT_ID = PEH.HOSPITAL_ACCOUNT_ID
...
FROM
	...
	INNER JOIN EMRDB.dbo.ENCOUNTER_DIAGNOSES PED ON PED.ENCOUNTER_ID = C.ENCOUNTER_ID
...
FROM
	...
	INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS PEH ON PEH.ENCOUNTER_ID = C.ENCOUNTER_ID
...
FROM
	...
	INNER JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY VPALH ON VPALH.ENCOUNTER_NUM = FTZ.ENCOUNTER_ID
	...
	OUTER APPLY
		...
		(
			...
			WHERE N.ENCOUNTER_NUM = VPALH.ENCOUNTER_NUM AND N.IN_DTTM < VPALH.IN_DTTM
...
FROM
	...
	LEFT JOIN #Base_Pop PEH ON PEH.ENCOUNTER_ID = B.ENCOUNTER_ID	
...
GROUP BY
	...
	B.ENCOUNTER_ID, peh.HOSP_DISCH_TIME
...
FROM
	...
	LEFT JOIN #Base_Pop PEH ON PEH.ENCOUNTER_ID = C.ENCOUNTER_ID
...
SELECT DISTINCT				
	...
	, CASE --if time zero is 24 hours after arrival, Outside Hospital should be reported as zero. Developer A updated this on 08.01.2019			
		...
		WHEN PEH.ADMIT_SOURCE_CODE IS NULL THEN 99		
		...
		WHEN PEH.ADMIT_SOURCE_CODE IN (106,23,4,95,6) THEN 1 		
	...
	, CASE			
		...
		WHEN PEH.DISCH_DISP_CODE IN ('1', '40', '50', '6', '81', '86') THEN 1		

		WHEN PEH.DISCH_DISP_CODE IN ('62', '90') THEN 2		

		WHEN PEH.DISCH_DISP_CODE IN ('20', '40', '41', '42') THEN 3		
	...
	, CASE	WHEN CRO.ENCOUNTER_ID IS NOT NULL THEN CONVERT(char(10), DATEADD(DD, 30, PEH.HOSP_ADMSN_TIME), 126) 		
	...
	, CASE			
		...
		WHEN CRO.ENCOUNTER_ID IS NOT NULL THEN CONVERT(char(10), DATEADD(DD, 30, PEH.HOSP_ADMSN_TIME), 126)		
...
FROM #COHORT C
	...
	LEFT JOIN EMRDB.dbo.PATIENTS PAT ON PAT.PATIENT_ID = PEH.PATIENT_ID
	...
	(
		...
		FROM 
			...
			INNER JOIN #IP_PositiveScores IPS ON IPS.ENCOUNTER_ID = PEH.ENCOUNTER_ID
		...
		WHERE
			...
			PEH.ENCOUNTER_ID = #ScreenTime.ENCOUNTER_ID
	...
	(--HUDDLE TIME FROM NOTES TKT-014
		...
		WHERE
			...
			HNO.ENCOUNTER_ID = #ScreenTime.ENCOUNTER_ID

			AND HNO.IP_NOTE_TYPE_CODE='1000007'--	Significant Event
