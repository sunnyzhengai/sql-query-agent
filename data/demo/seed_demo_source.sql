...
FROM [EMRDB].[dbo].ED_ENCOUNTERS_FACT EEF



	INNER JOIN [EMRDB].[dbo].HOSPITAL_ENCOUNTERS HE ON EEF.ENCOUNTER_ID = HE.ENCOUNTER_ID

	INNER JOIN [EMRDB].[dbo].ED_ENCOUNTERS_DM EED ON EED.ENCOUNTER_ID = EEF.ENCOUNTER_ID

	INNER JOIN [EMRDB].[dbo].PATIENTS PAT ON PAT.PATIENT_ID = HE.PATIENT_ID

	LEFT OUTER JOIN [EMRDB].[dbo].REF_ED_DISPOSITION REDI ON REDI.ED_DISPOSITION_CODE = HE.ED_DISPOSITION_CODE
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.MEDICATIONS ERX ON [ERX].MEDICATION_ID = MO.MEDICATION_ID
...
WHERE 1=1
	...
	AND MA.TAKEN_TIME < B.ED_DEPARTURE_TIME	-- while in ED
...
(
	...
	FROM #Base_Pop
		...
		INNER JOIN [EMRDB].[dbo].LINE_DEVICE_AIRWAY LNA ON LNA.ENCOUNTER_ID = [#Base_Pop].ENCOUNTER_ID
...
SELECT
	...
	--, ROW_NUMBER() OVER(PARTITION BY [#Base_Pop].ENCOUNTER_ID ORDER BY PO.ORDER_TIME ASC) AS TIME_LINE
...
FROM #Base_Pop
	...
	INNER JOIN [EMRDB].[dbo].PROCEDURE_ORDERS PO ON PO.ORDER_PROC_ID = LAB_ORDER_RESULTS.ORDER_PROC_ID

WHERE 1=1

	AND (PO.ORDER_TIME BETWEEN [#Base_Pop].ADT_ARRIVAL_TIME AND [#Base_Pop].ED_DEPARTURE_TIME)
	...
	AND (
			...
			OR (
				...
				PO.SPECIMEN_SOURCE_CODE = 304	-- Lumber puncture
...
FROM #Base_Pop B
	...
	LEFT OUTER JOIN [EMRDB].[dbo].ALERT_ACTIONS AA ON AA.ALT_ENCOUNTER_ID = AH.ALT_ENCOUNTER_ID AND AA.LINE=1

	LEFT OUTER JOIN [EMRDB].[dbo].REF_ALERT_ACTIONS RAA ON RAA.ALT_ACTION_TAKEN_CODE = AA.ACTION_TAKEN_CODE

	LEFT OUTER JOIN [EMRDB].[dbo].REF_ALERT_OVERRIDE_REASONS RAOR ON RAOR.ALRT_SP_OVR_RSN_CODE = AH.SPEC_OVR_RSN_CODE

	LEFT OUTER JOIN [EMRDB].[dbo].REF_ALERT_STATUS RAS ON RAS.ALT_STATUS_CODE = AH.ALT_STATUS_CODE

	LEFT OUTER JOIN [EMRDB].[dbo].REF_SHOWN_PLACE RSP ON RSP.SHOWN_PLACE_CODE = AH.SHOWN_PLACE_CODE
...
FROM #Base_Pop B
	...
	INNER JOIN [EMRDB].[dbo].ED_PATIENT_INFO EPI ON EPI.ENCOUNTER_ID = B.ENCOUNTER_ID

	INNER JOIN [EMRDB].[dbo].ED_EVENT_INFO EEI ON EEI.EVENT_ID = EPI.EVENT_ID AND EEI.EVENT_TYPE IN ('2600000347','2600000346')

	INNER JOIN [EMRDB].[dbo].ED_EVENT_TEMPLATES C ON EEI.EVENT_TYPE = C.RECORD_ID
...
FROM #Base_Pop
	...
	INNER JOIN [EMRDB].[dbo].ED_ENCOUNTERS_DM EED ON EED.PATIENT_ID = [#Base_Pop].PATIENT_ID 

											AND EED.ARRIVAL_DTTM BETWEEN ED_DEPARTURE_TIME AND DATEADD(HH, 24, ED_DEPARTURE_TIME)
...
FROM #ED_PositiveScores
	...
	INNER JOIN [EMRDB].[dbo].ED_ENCOUNTERS_DM EED ON EED.PATIENT_ID = [#Base_Pop].PATIENT_ID 
											...
											AND EED.ARRIVAL_DTTM BETWEEN [#Base_Pop].ED_DEPARTURE_TIME AND DATEADD(HH, 24, [#Base_Pop].ED_DEPARTURE_TIME)
...
SELECT DISTINCT   CAT.ENCOUNTER_ID,
        ...
        STUFF((	SELECT ';' + CONVERT(VARCHAR,VR.REASON_VISIT_NAME)-- AS [text()]
                ...
                FROM #Base_Pop SUB
					...
					INNER JOIN [EMRDB].[dbo].VISIT_REASONS VR ON VR.REASON_VISIT_ID = RSN.ENC_REASON_ID
...
SELECT
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND [BLD].MBOrderTime IS NOT NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [SSS SEVERE and Positive Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND [BLD].MBOrderTime IS NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END					AS [SSS SEVERE and Positive Sepsis and Blood Culture NOT Ordered]
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND [BLD].MBOrderTime IS NOT NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END			AS [SSS NON SEVERE and Positive Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NOT NULL AND [BLD].MBOrderTime IS NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [SSS NON SEVERE and Positive Sepsis and Blood Culture NOT Ordered]
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND [BLD].MBOrderTime IS NOT NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END			AS [SSS SEVERE and Negative Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND [BLD].MBOrderTime IS NULL AND SEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [SSS SEVERE and Negative Sepsis and Blood Culture NOT Ordered]
	...
	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND [BLD].MBOrderTime IS NOT NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END			AS [SSS NON SEVERE and Negative Sepsis and Blood Culture Ordered]

	,CASE WHEN [#ED_PositiveScores].MEAS_VALUE IS NULL AND ALLSCORES.AllSepsis_Scores IS NOT NULL AND [BLD].MBOrderTime IS NULL AND NONSEVERE.ENCOUNTER_ID IS NOT NULL THEN 1 ELSE 0 END				AS [SSS NON SEVERE and Negative Sepsis and Blood Culture NOT Ordered]
...
FROM #Base_Pop BasePop
	...
	LEFT OUTER JOIN [EMRDB].[dbo].VISIT_REASONS	VR			ON VR.REASON_VISIT_ID = CHIEF_CMPLNT.ENC_REASON_ID
...
(
	...
	INNER JOIN [EMRDB].[dbo].[HOSPITAL_ENCOUNTERS] HENC ON ENCS.ENCOUNTER_ID = HENC.ENCOUNTER_ID /*Developer C minimize to ENC_ID's we are looking at*/

	INNER JOIN [EMRDB].[dbo].[ADT_EVENTS] adtIn ON adtIn.ENCOUNTER_ID = HENC.ENCOUNTER_ID
...
INNER JOIN [EMRDB].[dbo].[HOSPITAL_ENCOUNTERS] HE ON HE.ENCOUNTER_ID = main.ENCOUNTER_ID

INNER JOIN  vaplh ON vaplh.ENCOUNTER_ID = HE.ENCOUNTER_ID AND vaplh.ADT_DEPARTMENT_ID IS NOT NULL /*[EMRDB].[dbo].V_PATIENT_LOCATION_HISTORY*/
...
INNER JOIN [EMRDB].[dbo].[FLOWSHEET_RECORDS] rec ON rec.INPATIENT_DATA_ID = HE.INPATIENT_DATA_ID
...
INNER JOIN [EMRDB].[dbo].[HOSPITAL_ENCOUNTERS] HE ON HE.ENCOUNTER_ID = bp.ENCOUNTER_ID
...
INNER JOIN [EMRDB].[dbo].[MEDICATION_ORDERS] MO	ON MO.ENCOUNTER_ID = base.ENCOUNTER_ID

INNER JOIN [EMRDB].[dbo].[MEDICATIONS] med ON med.MEDICATION_ID = MO.MEDICATION_ID AND med.THERA_CLASS_CODE = 11 /*Antibiotics*/
...
INNER JOIN [EMRDB].[dbo].[MED_ADMIN_RECORDS] mar ON mar.ORDER_MED_ID = MO.ORDER_MED_ID
...
INNER JOIN [EMRDB].[dbo].ORDER_TRACKING_METRICS MO ON MO.ENCOUNTER_ID = base.ENCOUNTER_ID

INNER JOIN [EMRDB].[dbo].[CALENDAR_DATES] dd ON dd.CALENDAR_DT = CAST(MO.ORDER_DTTM AS Date)
...
INNER JOIN [EMRDB].[dbo].[MEDICATIONS] med ON med.MEDICATION_ID = MO.MEDICATION_ID
...
INNER JOIN [EMRDB].[dbo].[LINE_DEVICE_AIRWAY] lda ON lda.ENCOUNTER_ID = b.ENCOUNTER_ID 
...
LEFT JOIN [EMRDB].[dbo].[MEDICATION_ORDERS] MO ON MO.ENCOUNTER_ID = base.ENCOUNTER_ID

LEFT JOIN [EMRDB].[dbo].[MEDICATIONS] MEDS ON MEDS.MEDICATION_ID = MO.MEDICATION_ID

LEFT JOIN [EMRDB].[dbo].GROUPER_MED_RECORDS gmr ON gmr.EXP_MEDS_LIST_ID = MEDS.MEDICATION_ID

LEFT JOIN [EMRDB].[dbo].[MED_ADMIN_RECORDS] mar ON mar.ORDER_MED_ID = MO.ORDER_MED_ID

LEFT JOIN [EMRDB].[dbo].[CALENDAR_DATES] dd ON dd.CALENDAR_DT = CAST(mar.TAKEN_TIME AS DATE)

LEFT JOIN [EMRDB].[dbo].[HOSPITAL_ENCOUNTERS] HE ON HE.ENCOUNTER_ID = base.ENCOUNTER_ID
...
FROM #Base_Pop base
	...
	INNER JOIN [EMRDB].[dbo].[LAB_ORDER_RESULTS] ordR ON base.ENCOUNTER_ID = ordR.ENCOUNTER_ID

	INNER JOIN [EMRDB].[dbo].[PROCEDURE_ORDERS] PO ON PO.ORDER_PROC_ID = ordR.ORDER_PROC_ID
...
INNER JOIN [EMRDB].[dbo].[CALENDAR_DATES] dd ON dd.CALENDAR_DT = CAST(PO.ORDER_TIME AS DATE)
...
INNER JOIN [EMRDB].[dbo].[PROCEDURE_ORDERS] PO  ON res.ORDER_PROC_ID = PO.ORDER_PROC_ID 

			AND PO.PROC_ID IN (SELECT * FROM #BloodCultures)

INNER JOIN [EMRDB].[dbo].[PROCEDURES_CATALOG] PCAT ON PCAT.PROC_ID = PO.PROC_ID
...
(	
	...
	INNER JOIN [EMRDB].[dbo].[PROVIDERS] PRV ON PRV.PROV_ID = tTeam.PROV_ID
...
(
	...
	FROM EMRDB.dbo.CLINICAL_NOTES CNOTE

		LEFT JOIN EMRDB.dbo.NOTE_TEMPLATE_TEXT_IDS etx ON etx.NOTE_ID = CNOTE.NOTE_ID

		LEFT JOIN EMRDB.dbo.NOTE_TEMPLATE_LIST_IDS lis ON lis.NOTE_ID = CNOTE.NOTE_ID

		INNER JOIN EMRDB.dbo.NOTE_ENCOUNTER_INFO hnoEnc ON hnoEnc.NOTE_ID = CNOTE.NOTE_ID
...
LEFT OUTER JOIN [reportingDB].[reports].[SEVERE_SEPSIS_STAGING] SSS ON SSS.ENCOUNTER_ID = main.ENCOUNTER_ID
...
FROM (
	...
	INNER JOIN [EMRDB].[dbo].[CLINICAL_ALERTS] alt ON alt.VISIT_ID = main.ENCOUNTER_ID AND alt.BPA_LOCATOR_ID in (900400001, 900400011) /*BASE 2019 HS OD SCORE SEPSIS >2 [900400001]*/
...
DROP TABLE IF EXISTS #SSOrderSetOSQ_PRL;
	...
	FROM #Base_Pop B
		...
		INNER JOIN EMRDB.dbo.ORDER_TRACKING_METRICS MO ON MO.ENCOUNTER_ID = B.ENCOUNTER_ID

		INNER JOIN [EMRDB].[dbo].MEDICATION_ORDERS_EXT MO2 ON MO2.ORDER_ID = MO.ORDER_ID AND MO2.ORD_OSQ_ID IN (400002,400007,400003,400004)
...
UNION
	...
	FROM #Base_Pop B
		...
		INNER JOIN [EMRDB].[dbo].PROCEDURE_ORDERS_EXT MO2 ON MO2.ORDER_ID = MO.ORDER_ID AND MO2.ORD_OSQ_ID IN (400002,400007,400003,400004)
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.MEDICATIONS MEDS ON MEDS.MEDICATION_ID = MO.MEDICATION_ID
...
FROM #Base_Pop C 
	...
	INNER JOIN [EMRDB].[dbo].FLOWSHEET_RECORDS FR ON FR.INPATIENT_DATA_ID = C.INPATIENT_DATA_ID

	INNER JOIN [EMRDB].[dbo].FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID
...
FROM #Base_Pop C
	...
	INNER JOIN [EMRDB].[dbo].LINE_DEVICE_AIRWAY LNA ON LNA.ENCOUNTER_ID = C.ENCOUNTER_ID 

	INNER JOIN reportingDB.reports.CONFIG_VALUE_SET CVS ON CVS.CODE = LNA.FLO_MEAS_ID
...
FROM #Base_Pop B
	...
	LEFT JOIN EMRDB.dbo.MEDICATION_ORDERS MO ON MO.ENCOUNTER_ID = B.ENCOUNTER_ID

	LEFT JOIN EMRDB.dbo.MEDICATIONS MEDS ON MEDS.MEDICATION_ID = MO.MEDICATION_ID

	LEFT JOIN EMRDB.dbo.GROUPER_MED_RECORDS GMR ON GMR.EXP_MEDS_LIST_ID = MEDS.MEDICATION_ID

	LEFT JOIN EMRDB.dbo.MED_ADMIN_RECORDS MA ON MA.ORDER_MED_ID = MO.ORDER_MED_ID

	LEFT JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS HE ON HE.ENCOUNTER_ID = B.ENCOUNTER_ID
...
FROM #Base_Pop B
	...
	INNER JOIN [EMRDB].[dbo].LAB_ORDER_RESULTS ON B.ENCOUNTER_ID = LAB_ORDER_RESULTS.ENCOUNTER_ID
...
FROM #Base_Pop B
	...
	INNER JOIN [EMRDB].[dbo].LAB_ORDER_RESULTS ON LAB_ORDER_RESULTS.ENCOUNTER_ID = B.ENCOUNTER_ID
...
(
	...
	FROM #Base_Pop B
		...
		INNER JOIN [EMRDB].[dbo].LAB_ORDER_RESULTS RESULTS ON B.ENCOUNTER_ID = RESULTS.ENCOUNTER_ID

		INNER JOIN [EMRDB].[dbo].PROCEDURE_ORDERS	 PO		 ON RESULTS.ORDER_PROC_ID = PO.ORDER_PROC_ID 
...
FROM #Base_Pop B
	...
	INNER JOIN [EMRDB].[dbo].LINE_DEVICE_AIRWAY LNA ON LNA.ENCOUNTER_ID = B.ENCOUNTER_ID
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY EMRDB_ADT_ED.EFFECTIVE_TIME ASC) AS TIME_LINE
...
FROM #Base_Pop B
	...
	INNER JOIN [EMRDB].[dbo].ADT_EVENTS EMRDB_ADT_ED 
			...
			ON EMRDB_ADT_ED.ENCOUNTER_ID =  B.ENCOUNTER_ID

				AND EMRDB_ADT_ED.DEPARTMENT_ID IN (200108022) 

				AND EMRDB_ADT_ED.EVENT_TYPE_CODE = 4 --TRANSFER OUT

				AND EMRDB_ADT_ED.EVENT_SUBTYPE_CODE <> 2 --CANCELED
	...
	INNER JOIN [EMRDB].[dbo].ADT_EVENTS EMRDB_ADT_GENCARE 
			...
			ON EMRDB_ADT_ED.XFER_IN_EVENT_ID = EMRDB_ADT_GENCARE.EVENT_ID
				...
				AND EMRDB_ADT_GENCARE.EVENT_TYPE_CODE = 3 --TRANSFER IN

				AND EMRDB_ADT_GENCARE.EVENT_SUBTYPE_CODE <> 2 --CANCELED
...
FROM #Base_Pop B
	...
	INNER JOIN [EMRDB].[dbo].ADT_EVENTS EMRDB_ADT_ED ON EMRDB_ADT_ED.ENCOUNTER_ID =  B.ENCOUNTER_ID
...
FROM #Base_Pop B
	...
	(
		...
		WHERE ICU.ENCOUNTER_ID = EMRDB_ADT_GENCARE.ENCOUNTER_ID
...
FROM #Base_Pop BP 
	...
	INNER JOIN [EMRDB].[dbo].FLOWSHEET_RECORDS FR ON HE.INPATIENT_DATA_ID = FR.INPATIENT_DATA_ID
...
FROM #BasePopABX A
	...
	INNER JOIN
		...
		(
			...
			[EMRDB].[dbo].ORDER_DISPENSE_INFO  ODI WHERE ODI.ORD_CNTCT_TYPE_CODE = 4--VERIFY
	...
	INNER JOIN
		...
		(
			...
			[EMRDB].[dbo].ORDER_DISPENSE_INFO  ODI WHERE ODI.ORD_CNTCT_TYPE_CODE = 95--DISPENSE
	...
	LEFT OUTER JOIN 
		...
		(
			...
			[EMRDB].[dbo].V_PHARMACY_DISPENSE_ACTION PDA WHERE PDA.ACTION_TYPE_CODE=270
...
FROM #Base_Pop BP
	...
	INNER JOIN [EMRDB].[dbo].ED_ENCOUNTERS_DM EED ON EED.PATIENT_ID = BP.PATIENT_ID AND (EED.ARRIVAL_DTTM BETWEEN BP.ED_DEPARTURE_TIME AND DATEADD(HH,24,BP.ED_DEPARTURE_TIME))
...
FROM #Base_Pop BasePop
	...
	(
		...
		FROM [EMRDB].[dbo].HOSPITAL_ENCOUNTERS
			...
			INNER JOIN [EMRDB].[dbo].ED_PATIENT_INFO		 ON [ED_PATIENT_INFO].ENCOUNTER_ID = [HOSPITAL_ENCOUNTERS].ENCOUNTER_ID
...
FROM  #Main sub
	...
	INNER JOIN EMRDB.dbo.ENCOUNTER_DIAGNOSES EDX ON EDX.ENCOUNTER_ID = SUB.ENCOUNTER_ID AND EDX.LINE>1
...
INNER JOIN EMRDB.dbo.PATIENTS PAT ON PAT.PATIENT_ID = #Main.PATIENT_ID
...
cross apply (
			...
			FROM
				...
				EMRDB..HOSPITAL_ENCOUNTERS HENC

				INNER JOIN EMRDB..ADT_EVENTS ADTIN ON ADTIN.ENCOUNTER_ID = HENC.ENCOUNTER_ID
				...
				left outer join EMRDB..BED_CONFIG bed on bed.BED_STAY_ID = adtin.BED_STAY_ID
...
FROM #Main BP 
	...
	INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS HE ON HE.ENCOUNTER_ID = BP.ENCOUNTER_ID --and bp.ENCOUNTER_ID=1016405505 

	INNER JOIN EMRDB.dbo.FLOWSHEET_RECORDS FR ON FR.INPATIENT_DATA_ID = HE.INPATIENT_DATA_ID

	INNER JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID and
...
FROM #Base_Pop BP 
	...
	LEFT OUTER JOIN
		...
		(
			...
			FROM EMRDB.dbo.FLOWSHEET_RECORDS FR

				INNER JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS IFM_ALERTNOTACTIVATED ON IFM_ALERTNOTACTIVATED.FSD_ID = FR.FSD_ID
...
FROM
	...
	INNER JOIN EMRDB.dbo.FLOWSHEET_RECORDS FR ON FR.INPATIENT_DATA_ID = B.INPATIENT_DATA_ID
	...
	(
		...
		EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM 
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.LINE_DEVICE_AIRWAY LNA ON LNA.ENCOUNTER_ID = B.ENCOUNTER_ID AND LNA.FLO_MEAS_ID='900112' AND LNA.PLACEMENT_INSTANT IS NOT NULL
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.LINE_DEVICE_AIRWAY LNA ON LNA.ENCOUNTER_ID = B.ENCOUNTER_ID AND LNA.FLO_MEAS_ID='900111' AND LNA.PLACEMENT_INSTANT IS NOT NULL
...
FROM #Base_Pop B
	...
	INNER JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID AND FM.RECORDED_TIME IS NOT NULL
...
EMRDB.dbo.V_PATIENT_LOCATION_HISTORY PLH

	INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS HE ON HE.ENCOUNTER_ID = PLH.ENCOUNTER_NUM

	INNER JOIN EMRDB.dbo.PATIENTS PAT ON PAT.PATIENT_ID = HE.PATIENT_ID

	INNER JOIN EMRDB.dbo.CALENDAR_DATES DT ON (DT.CALENDAR_DT BETWEEN CONVERT(DATE,PLH.IN_DTTM) AND CONVERT(DATE,PLH.OUT_DTTM))

	left outer join EMRDB.dbo.DEPARTMENTS dep on dep.DEPARTMENT_ID = PLH.ADT_DEPARTMENT_ID
...
WHERE
	...
	PLH.ADT_DEPARTMENT_ID IN (200108013,200108014)

	AND HE.INP_ADM_DATE IS NOT NULL
	...
	(
		...
		SELECT  top 1
            ...
            STUFF((    SELECT '; ' +PRV.PROV_NAME-- AS [text()]
						...
						INNER JOIN EMRDB.dbo.PROVIDERS PRV ON PRV.PROV_ID = SUB.PROV_ID
...
SELECT  DISTINCT   CAT.ENCOUNTER_ID,
            ...
            STUFF((    SELECT '% ' +DIAG.DX_NAME-- AS [text()]
...
INNER JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY VAPLH ON VAPLH.ENCOUNTER_NUM = HE.ENCOUNTER_ID AND VAPLH.ADT_DEPARTMENT_ID IS NOT NULL
...
SELECT
	...
	, CASE
		...
		WHEN 
			...
			B.AGE_MONTHS < 2 AND LEFT(FM.MEAS_VALUE, CHARINDEX('/', FM.MEAS_VALUE)-1) < 65

			OR

			(B.AGE_MONTHS >= 2 AND B.AGE_MONTHS < 12) AND LEFT(FM.MEAS_VALUE, CHARINDEX('/', FM.MEAS_VALUE)-1) < 70

			OR

			(B.AGE_YEARS >= 1 AND B.AGE_YEARS < 2) AND LEFT(FM.MEAS_VALUE, CHARINDEX('/', FM.MEAS_VALUE)-1) < 80

			OR

			(B.AGE_YEARS >= 2 AND B.AGE_YEARS < 6) AND LEFT(FM.MEAS_VALUE, CHARINDEX('/', FM.MEAS_VALUE)-1) < 90

			OR

			(B.AGE_YEARS >= 6 AND B.AGE_YEARS < 13) AND LEFT(FM.MEAS_VALUE, CHARINDEX('/', FM.MEAS_VALUE)-1) < 100

			OR

			B.AGE_YEARS >= 13 AND LEFT(FM.MEAS_VALUE, CHARINDEX('/', FM.MEAS_VALUE)-1) < 110

		THEN FM.MEAS_VALUE--FM.RECORDED_TIME
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY FM.RECORDED_TIME ASC) AS TIME_LINE
...
FROM
	...
	INNER JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID AND FM.FLO_MEAS_ID = '95' 

WHERE 

	FM.RECORDED_TIME IS NOT NULL

	AND (FM.RECORDED_TIME BETWEEN @dStartDate AND @dEndDate)
...
SELECT
	...
	, ROW_NUMBER() OVER(PARTITION BY MO.ENCOUNTER_ID ORDER BY MA.TAKEN_TIME) TIME_LINE
...
WHERE
	...
	AND (MA.TAKEN_TIME BETWEEN @dStartDate AND @dEndDate)
...
--SELECT * FROM #BasePopABX where ENCOUNTER_ID=1016093600--1015370131



--SELECT * FROM #BasePopABX where ENCOUNTER_ID=1016093600--1015370131

--select * from #Base_Pop_OD_Scores where ENCOUNTER_ID=1016093600
...
SELECT
	...
	--, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID, B.ADT_DEPARTMENT_ID, B.IN_DTTM ORDER BY MO.ORDER_DTTM ASC) AS TIME_LINE

	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID ORDER BY MO.ORDER_DTTM ASC) AS TIME_LINE
...
WHERE
	...
	AND (MO.ORDER_DTTM BETWEEN @dStartDate AND @dEndDate)

	--AND MO.ORDER_DTTM BETWEEN B.IN_DTTM AND B.OUT_DTTM
...
SELECT DISTINCT
	...
	, ROW_NUMBER() OVER(PARTITION BY B.ENCOUNTER_ID, B.ADT_DEPARTMENT_ID, B.IN_DTTM ORDER BY LNA.PLACEMENT_INSTANT) TIME_LINE
...
WHERE (LNA.PLACEMENT_INSTANT BETWEEN @dStartDate AND @dEndDate)

--WHERE LNA.PLACEMENT_INSTANT BETWEEN B.IN_DTTM AND B.OUT_DTTM
...
AND (PO.ORDER_TIME BETWEEN @dStartDate AND @dEndDate)
...
SELECT
	...
	--, PO.PROC_CODE AS [Blood Culture Procedure Ordered]--PROCEDURE_ORDERS.PROC_CODE IS DEPRICATED
...
WHERE (PO.ORDER_TIME BETWEEN @dStartDate AND @dEndDate)
...
SELECT
	...
	--, PO.PROC_CODE as [CSF Procedure Ordered]--PROCEDURE_ORDERS.PROC_CODE DEPRICATED
...
INNER JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID AND FM.RECORDED_TIME IS NOT NULL
	...
	--AND (FM.RECORDED_TIME BETWEEN B.IN_DTTM AND B.OUT_DTTM)

WHERE (FM.RECORDED_TIME BETWEEN @dStartDate AND @dEndDate)
...
IF OBJECT_ID(N'tempdb..#LacticAcidLRRByName') IS NOT NULL DROP TABLE #LacticAcidLRRByName;
	...
	FROM [EMRDB].[dbo].[LAB_COMPONENTS] LC
...
INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS HE ON HTR.ENCOUNTER_ID = HE.ENCOUNTER_ID AND HE.ADMIT_CONF_STAT_CODE IN (1,4) AND HE.HOSP_ADMSN_TIME IS NOT NULL AND HE.ADT_PATIENT_CLASS_CODE <> 102

INNER JOIN EMRDB.dbo.PATIENTS PAT ON PAT.PATIENT_ID = HE.PATIENT_ID

INNER JOIN EMRDB.dbo.HOSPITAL_ACCOUNTS HAR ON HAR.HOSPITAL_ACCOUNT_ID = HTR.HOSPITAL_ACCOUNT_ID

INNER JOIN EMRDB.dbo.CALENDAR_DATES dd ON dd.CALENDAR_DT = CAST(HE.HOSP_DISCH_TIME AS DATE)

WHERE HTR.SERVICE_DATE BETWEEN @StartDate AND @EndDate 

AND (

	HAR.ACCT_BASECLS_HA_CODE IN (1,3)--EXCLUDE OUTPATIENTS

	OR

	HAR.ACCT_CLASS_HA_CODE = 104--BUT INCLUDE OBSERVATION PATIENTS

)

AND HTR.TX_TYPE_HA_CODE = 1

AND HAR.ACCT_BILLSTS_HA_CODE <> 99 --Combined HAR (Not the primary)
...
AND htr.HOSPITAL_ACCOUNT_ID NOT IN (SELECT HOSPITAL_ACCOUNT_ID FROM EMRDB.dbo.HOSPITAL_ACCOUNTS_EXT WHERE AT_BILLING_PRIM_PAYER_ID = 2109) 
...
INNER JOIN EMRDB.dbo.ADT_EVENTS ADT ON ADT.ENCOUNTER_ID = B.ENCOUNTER_ID
...
AND FP.ENCOUNTER_ID  NOT IN (SELECT DISTINCT ENCOUNTER_ID FROM reportingDB.reports.SEVERE_SEPSIS_STAGING)--make sure we are not includiung already submitted encounters
...
INNER JOIN EMRDB.dbo.GROUPER_MED_RECORDS GMR ON GMR.EXP_MEDS_LIST_ID = MEDS.MEDICATION_ID
...
INNER JOIN EMRDB.dbo.PROCEDURE_ORDERS PO ON PO.ENCOUNTER_ID = b.ENCOUNTER_ID
...
INNER JOIN EMRDB.dbo.PATIENT_ENCOUNTERS enc on enc.ENCOUNTER_ID = bp.ENCOUNTER_ID
...
JOIN EMRDB.dbo.FLOWSHEET_RECORDS FR ON FR.INPATIENT_DATA_ID = enc.INPATIENT_DATA_ID

JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID
...
IF OBJECT_ID(N'tempdb..#SSOrderSetOSQ_PRL') IS NOT NULL DROP TABLE #SSOrderSetOSQ_PRL;
	...
	INNER JOIN EMRDB.dbo.MEDICATION_ORDERS_EXT MO2 ON MO2.ORDER_ID = MO.ORDER_ID 
...
UNION
	...
	INNER JOIN EMRDB.dbo.PROCEDURE_ORDERS_EXT MO2 ON MO2.ORDER_ID = MO.ORDER_ID 
...
/*TREATMENT PLAN*/
		...
		JOIN EMRDB.dbo.PROCEDURE_ORDERS PO ON PO.ENCOUNTER_ID = B.ENCOUNTER_ID
			...
			AND MO.MEDICATION_ID IN 
				...
				(
					...
					FROM (
						...
						--reportingDB.reports.CONFIG_VALUE_SET cntl on cntl.VALUE_SET_ID = 3016 and cntl.CODE = gen.SIMPLE_GENERIC_CODE -- and cntl.VALUE_SET_ABBR='Antibacterial'
		...
		JOIN EMRDB.dbo.MEDICATION_ORDERS MO ON MO.ENCOUNTER_ID = C.ENCOUNTER_ID

		JOIN EMRDB.dbo.MEDICATIONS MEDS ON MEDS.MEDICATION_ID = MO.MEDICATION_ID

		JOIN EMRDB.dbo.MED_ADMIN_RECORDS MA ON MA.ORDER_MED_ID = MO.ORDER_MED_ID
		...
		INNER JOIN EMRDB.dbo.MEDICATION_ORDERS MO ON MO.ENCOUNTER_ID = SC.ENCOUNTER_ID

		INNER JOIN EMRDB.dbo.MEDICATIONS MEDS ON MEDS.MEDICATION_ID = MO.MEDICATION_ID

		INNER JOIN #MedGroupersERX erx ON erx.ERX_ID = MEDS.MEDICATION_ID

		--INNER JOIN EMRDB.dbo.GROUPER_MED_RECORDS GMR ON GMR.EXP_MEDS_LIST_ID = MEDS.MEDICATION_ID 	AND
		...
		INNER JOIN EMRDB.dbo.MED_ADMIN_RECORDS MA ON MA.ORDER_MED_ID = MO.ORDER_MED_ID AND MA.ROUTE_CODE = 11  --INTRAVENOUS
...
JOIN EMRDB.dbo.ORDER_TRACKING_METRICS MO ON MO.ORDER_ID = PO.ORDER_PROC_ID --AND PO.ORDER_STATUS_CODE<>4-- AND PO.INSTANTIATED_TIME IS NOT NULL AND PO.FUTURE_OR_STAND IS NULL-- ENCOUNTER_ID = B.ENCOUNTER_ID
...
/*CRITERIA 4*/
	...
	INNER JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY PLH ON PLH.ENCOUNTER_NUM = b.ENCOUNTER_ID
...
JOIN EMRDB.dbo.PROCEDURE_ORDERS PO ON bp.ENCOUNTER_ID = PO.ENCOUNTER_ID

	AND PO.INSTANTIATED_TIME IS NOT NULL AND PO.FUTURE_OR_STAND IS NULL

JOIN EMRDB.dbo.LAB_ORDER_RESULTS res ON PO.ORDER_PROC_ID = res.ORDER_PROC_ID
...
IF OBJECT_ID(N'tempdb..#C7') IS NOT NULL DROP TABLE #C7;
	...
	JOIN EMRDB.dbo.HOSPITAL_ACCT_DIAGNOSES dx ON dx.HOSPITAL_ACCOUNT_ID = bp.HOSPITAL_ACCOUNT_ID

	JOIN EMRDB.dbo.DIAGNOSES DIAG ON DIAG.DX_ID = dx.DX_ID 
...
UNION
	...
	JOIN EMRDB.dbo.ENCOUNTER_DIAGNOSES dx ON dx.ENCOUNTER_ID = bp.ENCOUNTER_ID
...
LEFT JOIN [reportingDB].[reports].[SEVERE_SEPSIS_STAGING] p ON p.ENCOUNTER_ID = c.ENCOUNTER_ID AND p.Reviewed = 1
...
LEFT JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY adt ON c.ENCOUNTER_ID = adt.ENCOUNTER_NUM 
...
LEFT JOIN EMRDB.dbo.FLOWSHEET_RECORDS FR ON FR.INPATIENT_DATA_ID = c.INPATIENT_DATA_ID

LEFT JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID AND FM.FLO_MEAS_ID = '95'
...
LEFT JOIN EMRDB.dbo.LAB_ORDER_RESULTS ord ON c.ENCOUNTER_ID = ord.ENCOUNTER_ID
...
FROM 
	...
	LEFT JOIN EMRDB.dbo.LAB_ORDER_RESULTS ON LAB_ORDER_RESULTS.ENCOUNTER_ID = C.ENCOUNTER_ID
...
FROM 
	...
	JOIN EMRDB.dbo.LAB_ORDER_RESULTS RESULTS ON C.ENCOUNTER_ID = RESULTS.ENCOUNTER_ID

	JOIN EMRDB.dbo.PROCEDURE_ORDERS PO ON RESULTS.ORDER_PROC_ID = PO.ORDER_PROC_ID 
...
FROM
	...
	INNER JOIN EMRDB.dbo.PATIENT_OR_ADM_LINK POAL ON POAL.OR_LINK_VISIT_ID = C.ENCOUNTER_ID

	INNER JOIN EMRDB.dbo.V_LOG_BASED VLB ON VLB.LOG_ID = POAL.LOG_ID

	INNER JOIN EMRDB.dbo.HOSPITAL_ACCT_PX_LIST HAPL ON HAPL.HOSPITAL_ACCOUNT_ID = HE.HOSPITAL_ACCOUNT_ID
...
FROM 
	...
	INNER JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FR.FSD_ID = FM.FSD_ID 
...
FROM
	...
	INNER JOIN EMRDB.dbo.ENCOUNTER_DIAGNOSES EDX ON EDX.ENCOUNTER_ID = C.ENCOUNTER_ID

	INNER JOIN EMRDB.dbo.EDG_CURRENT_ICD10 ECI ON ECI.DX_ID = EDX.DX_ID
...
FROM
	...
	INNER JOIN EMRDB.dbo.EDG_CURRENT_ICD10 ECI ON ECI.DX_ID = EDX.DX_ID AND ECI.CODE IN ('Q89.01', 'Z90.81') --ASPLENIA CODES
...
FROM
	...
	INNER JOIN EMRDB.dbo.HOSPITAL_ENCOUNTERS HE ON HE.ENCOUNTER_ID = C.ENCOUNTER_ID

	JOIN EMRDB.dbo.DM_ICU_STAY ISM ON ISM.ENCOUNTER_ID = C.ENCOUNTER_ID
...
FROM
	...
	INNER JOIN EMRDB.dbo.V_PATIENT_LOCATION_HISTORY PLH ON PLH.ENCOUNTER_NUM = FTZ.ENCOUNTER_ID
...
FROM 
	...
	JOIN EMRDB.dbo.MEDICATION_ORDERS OMED ON OMED.ENCOUNTER_ID = C.ENCOUNTER_ID

	JOIN EMRDB.dbo.MED_ADMIN_RECORDS MAR ON MAR.ORDER_MED_ID = OMED.ORDER_MED_ID 

	JOIN EMRDB.dbo.MEDICATIONS MED ON MED.MEDICATION_ID = OMED.MEDICATION_ID AND MED.THERA_CLASS_CODE = 11 --ANTIBIOTICS
...
FROM
	...
	LEFT JOIN EMRDB.dbo.FLOWSHEET_MEASUREMENTS FM ON FM.FSD_ID = FR.FSD_ID
...
FROM #COHORT C
	...
	LEFT JOIN EMRDB.dbo.PATIENTS PAT ON PAT.PATIENT_ID = HE.PATIENT_ID
	...
	(--HUDDLE TIME FROM NOTES TKT-014
		...
		FROM 
			...
			EMRDB.dbo.CLINICAL_NOTES CNOTE			

			INNER JOIN EMRDB.dbo.HNO_NOTE_TEXT HNT ON HNT.NOTE_ID = CNOTE.NOTE_ID			
