...
# FUZZER-1 stage (dev, per the order): the automated Sunny — N
# fresh phrasings per intent; findings are lexicon food
...
echo "  - $FUZZV" >> internal/docs/NIGHTLY_BATTERY.md
echo "$FUZZ" | grep "^  - " >> internal/docs/NIGHTLY_BATTERY.md 2>/dev/null
