...
After running: 700 (re-embed) then 800 (export) run next — the export
ORDER FIX (renumbering 2026-08-18) means descriptions land in the LPG
in the normal flow; the old re-run-05 double-run is gone.
"""
...
require_engine(src.__version__, REQUIRES_ENGINE, "600_generate_descriptions")
...
# using the findings 200_parse persisted (which carry steward dispositions —
# a steward's 'redact' decision must hold here). 200 always writes the
# table, even when empty, so absence means 200 never ran: STOP. The old inline
...
print("\nNext: 610 (agent descriptions) or 700_refresh_search_index -> 800_export_graph_tables.")
