...
| spec:B1 | witness totality (anti-fabrication) | axm:B1 | witness totality IS 'no claim without a witness' | 0005, 0044, 0048 |
| spec:B2 | description provenance is total and closed | axm:B1, axm:J4 | provenance closed -> every description judged | 0044 |
| spec:C1 | the frontier is enumerated | axm:D1 | the enumerated frontier -> nothing unreachable | 0001, 0048, 0052, 0053, 0054 |
| spec:C2 | conservation per extractor | axm:R1 | handled + fallout = total (conservation) | 0041, 0044, 0045, 0053 |
| spec:C3 | images land in the graph | axm:R1 | handled + fallout = total (conservation) | 0039, 0042 |
| spec:C4 | leaf grounding (termination) | axm:R1, axm:D1 | leaf grounding: termination + reachability | 0014, 0047 |
...
| spec:E2 | replay determinism for retrieval | axm:J2 | replay determinism = the computable type | 0032, 0054, 0055 |
| spec:E3 | the decision typing rule | axm:M5, axm:J2 | the decision-typing rule, verbatim | 0035, 0050, 0051 |
| spec:E4 | pick containment | axm:M5 | intent decisions bind to the human | 0044, 0046 |
| spec:E5 | filter grounding | axm:B1 | filter values need witnesses | 0044, 0046 |
| spec:E6 | presentation honesty | axm:B2, axm:B3 | boundary honesty + bounded quantified claims | 0036, 0044, 0051 |
| spec:F | the round trip (0044 as equations) | axm:J4 | the round trip is the description's oracle | 0044 |
| spec:G1 | one owner per capability | axm:D2 | one owner per capability, mechanized | 0047 |
| spec:G2 | sanctioned powers only (Uses \ S = empty) | axm:D2 | one owner per capability, mechanized | 0001, 0047 |
...
| spec:P1 | one conversation decides a turn | axm:M2 | one mind, full evidence | 0051 |
| spec:P2 | full tool results persist in one history | axm:M2 | one mind, full evidence | 0051 |
| spec:P3 | thinking room | axm:M3 | thinking room | 0051 |
| spec:P4 | no question-family casebook | axm:M4 | no question-shaped control flow | 0051 |
...
| spec:R7 | confirmed SQL only; read-only by construction | axm:B4 | confirmed-only execution | 0061 |
...
| spec:T1 | descriptions round-trip (spec:F as family member) | axm:J4 | descriptions - blind verifier + kappa-diff | 0065 |
| spec:T2 | SQL stitching round-trips | axm:J4, axm:B1 | SQL stitching - parseability round-trips; kappa-diff is the stated gap | 0065 |
| spec:T3 | definition creation round-trips | axm:M5, axm:J2 | definition creation - the human is the judge (L3 stratum) | 0065 |
