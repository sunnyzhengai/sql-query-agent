...
114 modules, 1234 tests, every module accounted: claimed by an ADR in the trace registry or declaring a law/contract in its docstring (`Proves:` line).
...
| 0035 | Agentic conversation over deterministic tools | `tests/orchestrator/test_agent.py` (6), `tests/orchestrator/test_tools.py` (21), `tests/webapp/test_app.py` (56) |
...
| 0060 | The parse is the plan: parser-only LLM, deterministic traversal, correction flywheel | `tests/orchestrator/test_parse_plan.py` (16) |
| 0061 | The run layer: Pro runs the confirmed definition | `tests/test_run_layer.py` (15) |
| 0062 | The dialogue loop: show, propose, ask, execute | `tests/webapp/test_app.py` (56) |
...
- `tests/orchestrator/test_parse_plan.py` (16): ADR 0060 prototype L0: closure is structural, grounding is exact,
