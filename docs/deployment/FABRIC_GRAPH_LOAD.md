...
4. For each of the 7 files: right-click → **Load to Tables** →
   **New table** → keep the file's name (`graph_db`,
   ...
   `graph_contains_dbSchema`, `graph_contains_schemaTable`,
   `graph_contains_tableColumn`). Result: 7 Delta tables.
...
7. Map **edge type `contains`** three times, `sourceId → targetId`:
   - db → schema via graph_contains_dbSchema
   - schema → table via graph_contains_schemaTable
   - table → column via graph_contains_tableColumn
...
```gql
MATCH ()-[c:contains]->() RETURN count(c)
```
...
```gql
MATCH (s:schema)-[:contains]->(t:table)
