...
Run BEFORE 300_build_graph so the consumption layer lands in the graph.
"""
...
require_engine(src.__version__, REQUIRES_ENGINE, "060_ingest_semantic_models")
...
partition_fallout = [f for f in out.fallout_rows if f["stage"] == "060_partition_parse"]
...
if sm.source_type == "workspace":
    for ws_id, r in ws_counts.items():
        for model, cls, reason in r["skipped"]:
            fallout_all.append({
                "run_at": RUN_AT, "stage": "060_collect",
