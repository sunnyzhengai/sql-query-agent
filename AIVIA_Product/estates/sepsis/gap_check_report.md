...
**Voicing ledger, corpus-wide:** 461 decisions voiced + 207 counted == 668 total (disjoint; every omission traces to a policy line or a census row).

**The gap taxonomy** (ruled-silent = ok forever, by ruling; open = needs resolution):
- RULED-SILENT: 303 (operational statements, degenerate predicates)
- OPEN, engine debt (ours): 2055 (unmapped kinds + unbound reference classes — shrinks with builds)
...
```
This step produces derived values; no source records are read.
- The recorded time is between the in dept date and the out dept date (inclusive).
```
...
```
This is a selection of records.
Drawn from config value set records.
...
```
This step produces derived values; no source records are read.
- The flo meas id is one of the values '9000161711', '9000002644' (annotated 'ORGAN DYSFUNCTION SCORE' in the source).
- The recorded time is between the in dttm and coalesce(bp.out_dttm,getdate()) (inclusive).
```
...
```
This is a selection of records.
Drawn from an inline selection, combined with an inline selection.
```
