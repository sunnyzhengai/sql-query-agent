...
**Voicing ledger, corpus-wide:** 469 decisions voiced + 262 counted == 731 total (disjoint; every omission traces to a policy line or a census row).

**The gap taxonomy** (ruled-silent = ok forever, by ruling; open = needs resolution):
- RULED-SILENT: 318 (operational statements, degenerate predicates)
- OPEN, engine debt (ours): 2275 (unmapped kinds + unbound reference classes — shrinks with builds)
- OPEN, estate findings (the customer's): 0 documentation gaps + 39 drift refs (columns nowhere declared — silently-failing reports, kept counted forever)
...
- The recorded time is on or before the ed departure time.
...
- The placement instant is between the adt arrival time and the ed departure time (inclusive).
...
This is a selection of records.
Drawn from the base pop selection defined earlier in this procedure, restricted to records also present in flowsheet records records, restricted to records also present in flowsheet measurements records.
- The flo meas id is one of the values '9000613042', '9000613043', '9000613044', '9000613045', '9000613047', '9000613048', '9000613050'.
- It is not the case that the recorded time has no recorded value.
```
...
- The exact moment when the reading was recorded is between the in dttm and coalesce(bp.out_dttm,getdate()) (inclusive).
```
