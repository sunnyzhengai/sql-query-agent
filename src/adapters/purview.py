...
class PurviewAdapter:
    ...
    def ensure_glossary(self, name: str = "AIVIA") -> "str | None":
        ...
        logger.error(f"ensure_glossary failed: HTTP {resp.status_code}: {resp.text[:200]}")
        return None

    def _entity_guid(self, type_name: str, qualified_name: str) -> "str | None":
        """Resolve an entity guid by unique attribute (qualifiedName)."""
        import requests
        resp = requests.get(
        ...
        if resp.status_code == 200:
            return (resp.json().get("entity") or {}).get("guid")
        return None

    def publish_glossary_term(
        self,
        glossary_guid: str,
        name: str,
        definition: str,
        ...
        see_also_guids: "list[str] | None" = None,
    ) -> PublishResult:
        """Create one glossary term and assign it to its implementing assets.

        One term = one definition (ADR 0031); sibling definitions are
        separate terms cross-linked via see_also_guids. Purview supports
        one term -> many assignedEntities natively. Weight/status travel
        in longDescription (term templates can carry them as managed
        attributes later).
        """
        import requests
        ...
        if see_also_guids:
            term["seeAlso"] = [{"termGuid": g} for g in see_also_guids]
        resp = requests.post(
            f"{self.base_url}/catalog/api/atlas/v2/glossary/term",
            headers=self._get_headers(), json=term, timeout=30,
        )
        if resp.status_code not in (200, 201):
            return PublishResult(
                asset_id=name, status=PublishStatus.FAILED,
                message=f"term create HTTP {resp.status_code}: {resp.text[:200]}",
            )
        term_guid = resp.json().get("guid")

        entity_refs = []
        missing = []
        etype = type_name or self.config.custom_type_name
        for qn in asset_qualified_names:
            guid = self._entity_guid(etype, qn)
            if guid:
                entity_refs.append({"guid": guid})
            else:
                missing.append(qn)
        if entity_refs:
            resp = requests.post(
                f"{self.base_url}/catalog/api/atlas/v2/glossary/terms/{term_guid}/assignedEntities",
                headers=self._get_headers(), json=entity_refs, timeout=30,
            )
            if resp.status_code not in (200, 204):
                return PublishResult(
                    asset_id=name, status=PublishStatus.FAILED,
                    message=f"assign HTTP {resp.status_code}: {resp.text[:200]}",
                )
        message = f"term {term_guid}: {len(entity_refs)} assets assigned"
        if missing:
            message += f"; {len(missing)} not found: {missing[:3]}"
        return PublishResult(
            asset_id=name, status=PublishStatus.SUCCESS, message=message,
        )
